...
export const StyledLabel = styled.Text<labelProps>`
    ...
    bottom: ${({ focused, value, placeholder, fixedLabel }) => focused || value || placeholder || fixedLabel  ? '40px' : '10px'};
    opacity: ${({ focused, value, placeholder,fixedLabel }) => focused || value || placeholder || fixedLabel  ? 1 : 0.7};
    ${({ rtl }) => rtl ? 'right' : 'left'}: 10px;
    position: ${({ fixedLabel }) => fixedLabel ? 'relative' : 'absolute'};
`;
